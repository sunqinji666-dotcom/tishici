...
ブラウザを開けばすぐ使える個人用プロンプト・メモ保管庫です。既定ではプロンプトを表示し、独立した「随手記」を選ぶと、文章・画像・動画を同じ2カラム画面で保存できます。
...
tishici は、個人の制作メモを保存する小さなセルフホスト型アーカイブです。プロンプトでは生成モデル、タイトル、本文、最大 12 個の画像・動画素材を1枚のカードにまとめます。「随手記」には、短いメモ、タスク、画像、動画を残すための独立した入力欄と一覧があります。

画面は写真家の暗室を思わせるダークモードで起動し、常にプロンプトを優先表示します。一覧は最初に6件を描画し、スクロールに合わせて追加読み込みします。検索、並べ替え、コピー、削除、半透明のメディアビューアも利用できます。
...
- **モード別に独立**：プロンプトと随手記はそれぞれ別のローカル下書きと一覧を持ち、切り替えても互いを上書きしません。
- **ブラウザで圧縮**：1 MiB 以下はそのまま、超過した素材だけをブラウザで変換します。サーバーは種類とサイズを検証するだけです。
- **画像と動画を混在可能**：生成モデルの選択は種類別のまま、どちらのプロンプト種類と随手記でも複数の画像・動画を一度に選択またはドロップできます。
- **素早く見返せる**：素材グループは3層のコンタクトシート風に表示され、左右ボタン、方向キー、下部フィルムストリップで切り替えられます。最初の6件からスクロールで次のまとまりを自動表示します。
...
現在のバージョンは `v0.2.0` です。[GitHub Releases](https://github.com/sunqinji666-dotcom/tishici/releases/latest) から `tishici-v0.2.0-web-source.zip` をダウンロードし、同名の `.sha256` で検証してください。

```bash
shasum -a 256 -c tishici-v0.2.0-web-source.zip.sha256
```

[CHANGELOG.md](../CHANGELOG.md) と [v0.2.0 Release ノート](RELEASE_NOTES_v0.2.0.md) も参照してください。
